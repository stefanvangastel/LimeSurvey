    ...
    public function templaterename()
    {
        if (returnGlobal('action') == "templaterename" && returnGlobal('newname') && returnGlobal('copydir')) {
            ...
            if (isStandardTemplate(returnGlobal('newname')))
                $this->getController()->error(sprintf(gT("Template could not be renamed to `%s`.", "js"), $newname) . " " . gT("This name is reserved for standard template.", "js"));
            elseif (rename($olddirname, $newdirname) == false)
                $this->getController()->error(sprintf(gT("Directory could not be renamed to `%s`.", "js"), $newname) . " " . gT("Maybe you don't have permission.", "js"));
            else
            {
    ...
    public function templatecopy()
    {
        ...
        if (returnGlobal('action') == "templatecopy" && returnGlobal('newname') && returnGlobal('copydir')) {
            ...
            elseif ($mkdirresult == 2)
                $this->getController()->error(sprintf(gT("Directory with the name `%s` already exists - choose another name", "js"), $newname));
            else
                $this->getController()->error(sprintf(gT("Unable to create directory `%s`.", "js"), $newname) . " " . gT("Please check the directory permissions.", "js"));
            ;
        }
    }
    ...
    public function delete($templatename)
    {
        ...
        if (is_template_editable($templatename) == true) {
            ...
            if (rmdirr(Yii::app()->getConfig('usertemplaterootdir') . "/" . $templatename) == true) {
                ...
                Yii::app()->session['flashmessage'] = sprintf(gT("Template '%s' was successfully deleted."), $templatename);
            }
            else
                Yii::app()->session['flashmessage'] = sprintf(gT("There was a problem deleting the template '%s'. Please check your directory/file permissions."), $templatename);
        }
    ...
    protected function _initialise($templatename, $screenname, $editfile, $showsummary = true)
    {
        ...
        switch ($screenname)
        {
            case 'surveylist':
                ...
                "contact" => sprintf(gT("Please contact %s ( %s ) for further assistance."), $siteadminname, $siteadminemail),
